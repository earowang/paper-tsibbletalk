...
hierarchical discovery arising from the data. It visually interprets
hierarchies in the shared tsibble's \texttt{spec} as a tree view, made
with \CRANpkg{plotly}. The following code line produces the linked tree
diagram and fills the left panel of Figure
\ref{fig:tourism-linking-fig}.
...
The visual of tree hierarchy untangles a group of related series and
snapshots the data organisation from a bird's eye view. It is
accompanied with other two plots to compose an ensemble of coordinated
graphics for multi-facetted exploration, shown as Figure
\ref{fig:tourism-linking-fig}. The time series and scatter plots of
Figure \ref{fig:tourism-linking-fig} are the interactive realisation of
Figure \ref{fig:highlight-retail}. Since all plots are derived from one
shared tsibble data source, they are self-linking multiple views. For
example, selecting the leaf node ``Melbourne'' fires up four respective
series of the \texttt{Purpose} category in other subplots,
characterising ``categorical linking''. Press the ``Shift'' key to
switch on persistent selection and simultaneously select the parent node
