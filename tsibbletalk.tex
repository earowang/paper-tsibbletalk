...
lower-level tourism regions. A new function \texttt{plotly\_key\_tree()}
has been implemented to address the need of hierarchical discovery
arising from data. It visually interprets hierarchies in the shared
tsibble's \texttt{spec} as a tree view, made with \CRANpkg{plotly}. From
a static point of view, a tree visualisation of hierarchical data
facilitates to grasp the overview structure. From the interactive
perspective, selecting a parent node along the tree subsequently
triggers its children and their corresponding series, or vice versa. It
makes navigation simpler than conditional panels, and also enables
persistent brushing for comparisons across groups.

\begin{Schunk}


\begin{center}\includegraphics[width=\textwidth]{img/tourism-linking} \end{center}
