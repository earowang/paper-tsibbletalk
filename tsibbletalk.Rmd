...
R \CRANpkg{shiny} and \CRANpkg{htmlwidgets} lay a fundamental infrastructure in marrying R with HTML elements and Javascript for interactivity, although they do not emphasise on interactive graphics. The \CRANpkg{htmlwidgets} package opens a door to embedding Javascript libraries into R such that users are able to write only R code to generate web-based plots. Many Javascript charting libraries have been ported to R as HTML widgets, including \CRANpkg{plotly}, \CRANpkg{rbokeh}, and \CRANpkg{leaflet} for maps. To enable interactions between different widgets, it can be achieved with \CRANpkg{shiny} or \CRANpkg{crosstalk}. The \CRANpkg{crosstalk} extends \CRANpkg{htmlwidgets} with shared R6 instances to support linked brushing and filtering across widgets.

If not using web technology, two examples are provided here: \CRANpkg{loon} based on Tcl/Tk and \CRANpkg{cranvas} based on Qt. They offer a wide array of pre-defined interactions, such as selecting and zooming, to manipulate plots via mouse, keyboard strokes, and menus. The \CRANpkg{cranvastime} is an add-on of \CRANpkg{cranvas}, which develops a set of novel interactions for temporal data, for example wrapping and mirroring.
