...
The shared tsibble data leverages the `key` attribute to converse with many coordinated views, with or without \CRANpkg{shiny}. On the other hand, a second critical attribute--`index`--lays the foundational temporal context that augments the conversation. When temporal data are plotted and stretched against the entire span like Figure \ref{fig:highlight-retail-1}, it puts the emphasis on the trend. When the data are wrapped over origin-less time units, such as one or two quarters, it makes seasonal patterns more pronounced. The \CRANpkg{tsibbletalk} package provides a shiny module
